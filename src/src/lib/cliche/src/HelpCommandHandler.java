...
//      List<ShellCommand> commands = owner.getCommandTable().getUserCommandTable();
...
//    }
    ...
    public List<String> list() {
      ...
      List<String> result = new ArrayList<>( commands.size() );
        for (ShellCommand command : commands) {
            if (command.getPrefix() == null || command.getPrefix().isEmpty() || command.getPrefix().equals( "?" ) ) {
                result.add(formatCommandShort(command));
            }
        }
        return result;
...
//      try ( OutputStreamWriter w = new FileWriter( file ) )
//      {
//        w.write( html );
//      }
...
//      List<ShellCommand> commands = owner.getCommandTable().commandsByName( commandName, false );
...
//    }
    ...
    private static String formatCommandShort(ShellCommand command) {
        ...
        String abbrevSpace = "         ";
        String nameSpace = "                      ";
      final String prefix = hasAbbr ? command.getPrefix() : "";
      final String abbrev = hasAbbr ? command.getAbbreviation() : "";

      return
              String.format(
              "%s%s" + abbrevSpace.substring( 0, abbrevSpace.length() - ( prefix.length() + abbrev.length() ) )
            + "%s%s" + nameSpace.substring( 0, nameSpace.length() - ( prefix.length() + command.getName().length() ) )
            + "%s", prefix, abbrev, command.getPrefix(), command.getName(), formatCommandParamsShort( command ) );
    }
